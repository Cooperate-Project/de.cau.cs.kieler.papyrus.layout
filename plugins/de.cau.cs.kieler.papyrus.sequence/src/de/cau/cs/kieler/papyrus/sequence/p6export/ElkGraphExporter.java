...
public final class ElkGraphExporter implements ISequenceLayoutProcessor {
    ...
    public void process(final LayoutContext context, final IElkProgressMonitor progressMonitor) {
        ...
        ElkUtil.resizeNode(context.kgraph,
                context.sgraph.getSize().x + 10,
                context.sgraph.getSize().y + 40,
                false,
                false);
        
        
        context.kgraph.setX(context.borderSpacing);
        context.kgraph.setY(context.borderSpacing);
    ...
    private void applyOutgoingMessageCoordinates(final SLifeline lifeline, final SMessage message,
        ...
        edgeSection.setStartX(llCenter);
        ...
        if (messageType == MessageType.LOST) {
            ...
            edgeSection.setEndX(lifeline.getPosition().x + lifeline.getSize().x + context.lifelineSpacing / 2);
    ...
    private void applyExecutionCoordinatesToSelfMessages(final SLifeline lifeline,
        ...
        for (final SMessage selfMsg : selfMsgs.keySet()) {
            ...
            ElkEdgeSection edgeSection = ElkGraphUtil.firstEdgeSection(selfMsgEdge, false, false);
            ...
            // Apply offsets
            offsetStartX(edgeSection, sourceOffset, context);
            offsetEndX(edgeSection, targetOffset, context);
            
            double maxOffset = Math.max(sourceOffset, targetOffset);
            offsetX(edgeSection.getBendPoints(), maxOffset, context);
     ...
     *            the layout context that contains all relevant information for the current layout run.
     */
    private void offsetX(final ElkBendPoint point, final double delta, final LayoutContext context) {
    ...
    /**
     * Adds the given delta to the given section's startX coordinate. Also ensures the graph is wide enough
     * to accomodate the new point.
     * 
     * @param point
     *            the point to offset.
     ...
     *            the layout context that contains all relevant information for the current layout run.
     */
    private void offsetStartX(final ElkEdgeSection section, final double delta, final LayoutContext context) {
        section.setStartX(section.getStartX() + delta);
        ensureGraphIsWideEnough(context, section.getStartX());
    }
    
    /**
     * Adds the given delta to the given section's endX coordinate. Also ensures the graph is wide enough
     * to accomodate the new point.
     * 
     * @param point
     *            the point to offset.
     ...
     *            the layout context that contains all relevant information for the current layout run.
     */
    private void offsetEndX(final ElkEdgeSection section, final double delta, final LayoutContext context) {
    ...
    /**
     * Calls {@link #offsetX(ElkBendPoint, double)} on every point in the given list. Also 
     * 
     * @param points
